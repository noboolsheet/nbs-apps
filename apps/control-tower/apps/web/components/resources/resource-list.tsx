...
export function ResourceList({ rows }: { rows: ResourceRow[] }) {
  if (rows.length === 0) return <p className="text-sm text-fg-subtle">{t('ui.sinActivosTodavia')}</p>;
  const columns: { header: string; className?: string; cell: (r: ResourceRow) => ReactNode }[] = [
    ...
    { header: t('ui.enlace'), cell: (r) => <ExternalSourceLink url={r.url} label={t('common.open')} /> },
    { header: t('ui.credencial'), cell: (r) => (r.credentialLocation ? <span className="text-xs text-fg-subtle">🔑 {r.credentialLocation}</span> : <span className="text-fg-subtle">—</span>) },
  ];
  return (
    <RecordTable
      columns={columns}
